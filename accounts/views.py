...
def home(request):
    ...
    out_of_delivered = orders.filter(status='Out of delivery').count()


    context = {'Orders':orders, 'Customers':customers, 'total_orders':total_orders, 'total_customers':total_customers,
               'delivered':delivered, 'pending':pending, 'out_of_delivered':out_of_delivered}
...
@login_required(login_url='login')
@allowed_users(allowed_roles=['admin'])
def customer(request, pk_tst):
    customer = Customer.objects.get(id = pk_tst)
    orders = customer.order_set.all()
    oreder_count = orders.count()

    myFilter = OrderFilter(request.GET, queryset=orders )
    orders = myFilter.qs
    context = {'customer':customer, 'orders':orders, 'oreder_count':oreder_count,'myFilter':myFilter}
